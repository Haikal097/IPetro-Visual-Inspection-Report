            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={handlePrint}
                  type="button"
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg font-medium bg-gray-800 hover:bg-gray-900 text-white shadow-sm transition"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
                ...
                {data.items.map((item, index) => (
                  <tr key={item.id} className="align-top group hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="p-4 relative border-r border-gray-200">
                      ...
                      <div className="aspect-[4/3] bg-white border-2 border-dashed border-gray-300 flex flex-col items-center justify-center relative group/img overflow-hidden transition-all hover:border-[#CD202C] mx-auto w-full rounded-lg hover:shadow-md">
                        {item.image ? (
                          <>
                            <img src={item.image} className="object-contain w-full h-full p-2" alt="" />
                            <button
                              type="button"
                              onClick={() => handleEditImage(item.id)}
                              className="absolute inset-0 bg-black/60 opacity-0 group-hover/img:opacity-100 transition-opacity flex flex-col items-center justify-center text-white font-semibold cursor-pointer no-print"
                            >
                          ...
                          <button
                            type="button"
                            onClick={() => handleEditImage(item.id)}
                            className="flex flex-col items-center text-gray-400 hover:text-[#CD202C] transition w-full h-full justify-center p-6"
                          >
                            <svg className="w-12 h-12 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                            </svg>
                            <span className="font-medium text-gray-600">Add Photo</span>
                            <span className="text-sm text-gray-500 mt-1">Click to upload or annotate</span>
                          </button>
                        )}
                      </div>
                    </td>
